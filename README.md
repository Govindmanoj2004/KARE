...
- **Patient pages**: check `isset($_SESSION['user_id'])` only — redirect to login if not. They do **not** check `role`, so a doctor account *can* technically browse patient URLs (harmless — data is scoped by `user_id` regardless of role, just semantically odd; not fixed, out of scope).
...
| **Password hashing** | Passwords are stored and compared in **plain text** everywhere (login, signup, change-password, deactivation) — explicit accepted tradeoff for this academic-scope project, not an oversight, but flag this loudly if this codebase is ever adapted for real use |
| **File access control** | Prescription files are served as static files at guessable-but-unguessable (random 32-hex-char) URLs, not through a session-checked streaming script |

---

## 7. Known Limitations / Accepted Tradeoffs

These are intentional, documented decisions — not bugs — but should be understood before extending the code:

1. **Plain-text passwords** everywhere (`password === $storedPassword` comparisons). Do not "fix" this without also handling migration of existing seeded plaintext passwords, and be aware `auth/login/controller.php`, `*/profile*_controller.php`, `*/account_controller.php`, and `*/settings_controller.php` all do direct string comparison.
2. **Editing a medicine's reminder times replaces the whole list** (`schedule_controller.php`'s `update_medicine`) — deletes and re-inserts `medicine_schedules`, which cascades and clears `dose_logs` history for removed times.
3. **Prescription files aren't access-gated** — anyone with the exact random URL can view a file without being logged in as that user. Acceptable because the filename is unguessable, not because it's actually access-controlled.
4. **N+1 queries in doctor's "My Patients"** page — one query per patient for adherence + today's doses. Fine for a small demo patient list; would need batching for scale.
5. **No pagination anywhere** — schedule lists, message threads, prescription lists, etc. all load in full. Fine at demo data volumes.
6. **Doctor role has no verification step** — any account with `role='doctor'` in the DB is treated as a legitimate doctor. There's no signup flow for doctors at all; the 3 demo doctors were seeded directly via SQL (`05_user_side_modules.sql`) because there was otherwise no way to create one.
...
## 10. Guidance for Continued Development (for other agents/models)
