...
> **Note:** §2/§3 below (project structure, file-by-file reference) predate the settings/reports/prescriptions/doctors/messages/help/search modules — all follow the exact same conventions described here (folder-per-feature, `$mrRootBase`, PRG + toast, shared modal for confirm-first actions). See `CHANGELOG.md` §10–§18 for what each one does and where it lives; this file's *rules* still apply to all of them unchanged.
